...
public class PlayerPrefsManager : MonoBehaviour
{
    ...
    public static void SetWallsDone(int index)
    {
        if (GetWallsDone() < index)
        {
            print("wall index is higher! " + index);
            PlayerPrefs.SetInt(WALLS_DONE_ANIMATED, index);
        }

    }
    public static int GetWallsDone()
    {
        return PlayerPrefs.GetInt(WALLS_DONE_ANIMATED, 0);
    }
    /*
     * Sound
     */
